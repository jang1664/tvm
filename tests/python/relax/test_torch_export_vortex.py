...
from tvm.relax.frontend.torch import from_exported_program  # pylint: disable=wrong-import-position
...
def test_default_vortex_pipeline_compiles_exported_mlp():
    ...
    assert "threadIdx.y" not in lowered_script
    assert "blockIdx.y" not in lowered_script
    assert 'scope="shared"' not in lowered_script
    assert 'scope="local"' not in lowered_script

    executable, source = _compile_without_device_toolchain(mod)
    assert executable.mod.kind == "relax.VMExecutable"
    assert source.count("// Vortex kernel") == 4
